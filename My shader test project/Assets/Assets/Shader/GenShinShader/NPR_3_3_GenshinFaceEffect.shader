Shader "Unlit/NPR_3_3_GenshinFaceEffect"
{
    Properties
    {
      
        _FaceShadow("Face Shadow Texture", 2D) = "white" {}
            ...
            float4 frag(v2f i) : SV_TARGET
            {
               ...
               // float3 NormalWS = mul(normalTS,TBN) ;
                ...
                //faceNRPeffect
                  ...
                  //identify right or left by 0/1
                  float switchShadow  = dot(normalize(Right.xz), normalize(LightDir.xz))*0.5+0.5 < 0.5;//why "<"
                  float SwitchSconvert = lerp(-1,1,switchShadow ) ;
                  float FaceShadoeTex = tex2D (_FaceShadow,float2(i.uv0.x * SwitchSconvert,i.uv0.y)) ;
                  //light gradient by light direction
                  ...
                  float FaceshadowSwitch = step(FaceShadowRange,FaceShadoeTex);


                  float3 var_Lut = tex2D(_Lut,float2(FaceshadowSwitch,_LuTV));
                ...
                float SelfLig = MaskMapValue.b;
                float specularHighlight = MaskMapValue.w;
                float3 Diffuse = DiffuseColor * var_Lut  ;
                ...
                //return float4(FaceshadowSwitch.xxx,1.0);
